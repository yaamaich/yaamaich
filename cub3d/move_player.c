...
/*   Updated: 2026/03/18 09:34:45 by yaamaich         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	position_blocks(t_game *g, double wx, double wy)
{
	...
	top = (int)(wy - 0.20);
	bottom = (int)(wy + 0.20);
	if (cell_blocks(g, left, top) || cell_blocks(g, right, top))
		return (1);
	if (cell_blocks(g, left, bottom) || cell_blocks(g, right, bottom))
		return (1);
	return (0);
}
